...
try:
    record = SeqIO.read(
        snakemake.input[0], "fasta"
    )  # Check if the fasta file has one record only
    SeqIO.write(
        record, snakemake.output[0], "fasta"
    )  # If only one record (i.e one contig) copy the file
    ...
    merged_contigs[0] = (
        ">" + headers[0] + "\n" + merged_contigs[0]
    )  # add the first header before the first contig
